...
* ``Results.num_interactions`` and ``Results.num_hb`` disagreed with the interactions actually stored. Several ligand atoms can fall inside one receptor atom's cutoff sphere, and the counts reported each of those pairs while the stored rows collapsed to one per receptor atom — so filtering on ``num_hb`` disagreed with the interaction table. The counts are now deduplicated the same way the rows are.
