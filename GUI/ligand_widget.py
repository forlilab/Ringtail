...
class LigandWidget(QtWidgets.QWidget):
    def __init__(self, active_filter=None):
        super(LigandWidget, self).__init__()
    ...
    def setupUi(self, Dialog, active_filter):
        ...
        self.gridLayout.addWidget(self.ligandNameRadioButton, 0, 5, 1, 2)
        ...
        self.gridLayout.addWidget(self.ligandSubstructureRadioButton, 0, 0, 1, 3)
        ...
        if self.ligand_name:
            ...
            if self.coordinatesCheckBox.isChecked():
                if self.coordinates_sanity_check():
                    ...
                    if self.xDoubleSpinBox.value() == '': 
                        self.ligand_filter.set_x(0.0)
                        self.xDoubleSpinBox.setValue(0.0)
                    else: self.ligand_filter.set_x(self.xDoubleSpinBox.value())
                    if self.yDoubleSpinBox.value() == '': 
                        self.ligand_filter.set_y(0.0)
                        self.yDoubleSpinBox.setValue(0.0)
                    else: self.ligand_filter.set_y(self.yDoubleSpinBox.value())
                    if self.zDoubleSpinBox.value() == '': 
                        self.ligand_filter.set_z(0.0)
                        self.zDoubleSpinBox.setValue(0.0)
                    else: self.ligand_filter.set_z(self.zDoubleSpinBox.value())
                    if self.cutoffDoubleSpinBox.value() == '' or self.cutoffDoubleSpinBox.value() < 1.0: 
                        self.ligand_filter.set_cutoff(1.0)
                        self.cutoffDoubleSpinBox.setValue(1.0)
                    else: self.ligand_filter.set_cutoff(self.cutoffDoubleSpinBox.value())
                    if self.indexSpinBox.value() == '': 
                        self.ligand_filter.set_index(0)
                        self.indexSpinBox.setValue(0)
                    else: self.ligand_filter.set_index(self.indexSpinBox.value())
            else:
                self.ligand_filter.set_coordinates(None, None, None)
                self.ligand_filter.set_cutoff(None)
                self.ligand_filter.set_index(None)
                print(self.ligand_filter)
        return self.ligand_filter

    def sanity_check(self):
        if self.ligand_name:
            return self.ligandNameLineEdit.text() != ''
        else:
            if not self.coordinatesCheckBox.isChecked():
                return self.substructureLineEdit.text() != ''
            else:
                return self.substructureLineEdit.text() != '' and self.coordinates_sanity_check() 
    ...
    def retranslateUi(self, Dialog):
        ...
        self.substructureLabel.setText(_translate("Dialog", "Substructure SMARTS:"))
