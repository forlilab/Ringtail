...
class StorageManagerSQLite(StorageManager):
    ...
    def _create_indices(self):
        ...
        try:
            ...
            cur.execute(
                "CREATE INDEX IF NOT EXISTS allind ON Interaction_indices(interaction_type,rec_chain,rec_resname,rec_resid)"
            )
            self.conn.commit()
            cur.close()
    ...
    def _generate_result_filtering_query(self, filters_dict):
        ...
        if self.interaction_cluster is not None:
            # TODO create a query that returns bitvectors for a given pose id
