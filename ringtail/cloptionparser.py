...
def cmdline_parser(defaults={}):
    ...
    subparsers = parser.add_subparsers(
        help="Specify if should write to or read from database",
        dest="rr_mode")

    write_parser = subparsers.add_parser('write',
                                         help="Write new files to database")
...
class CLOptionParser():
    ...
    def __init__(self):
        ...
        if len(self.lig_files_pool) == 0 and len(
                self.rec_files_pool) != 0 and self.save_receptor:
            self.db_opts["write_db_flag"] = False
            # raise error if not input db not given
            if self.input_db is None:
                raise OptionError(
                    "No input database given for saving receptor(s)")
        if len(self.lig_files_pool) == 0 and (self.db_opts["write_db_flag"]
                                              or self.db_opts["add_results"]):
            raise OptionError(
    ...
    def process_options(self, parsed_opts):
        ...
        if self.mode not in allowed_modes:
            raise OptionError(
                "Given mode {0} not allowed. Please be sure that requested mode is 'vina' or 'dlg'"
                .format(self.mode))
        if self.mode == "vina":
        ...
        if self.rr_mode == "write":
            ...
            file_sources = {
            }  # 'file':None, 'files_path':None, 'file_list':None}
            ...
            if (file_sources['file'] is
                    None) and (file_sources['file_path'] is None) and (
                        file_sources['file_list'] is
            ...
            if parsed_opts.max_miss < 0:
                raise OptionError(
                    "--max_miss must be greater than or equal to 0")
            if parsed_opts.max_miss > 0:
                if parsed_opts.plot:
                    raise OptionError(
            ...
            if properties["eworst"] is not None and properties[
                    "energy_percentile"] is not None:
                warnings.warn(
                    "Cannot use --eworst cutoff with --energy_percentile. Overiding energy_percentile with eworst."
                )
                properties["energy_percentile"] = None
            if properties["leworst"] is not None and properties[
                    "le_percentile"] is not None:
                warnings.warn(
            ...
            interactions = {}
            res_interactions_kw = [('van_der_waals', 'V'),
                                   ('hydrogen_bond', 'H'),
                                   ('reactive_res', 'R')]
            for opt, _type in res_interactions_kw:
                ...
                if res_list is None:
                    ...
                    warnings.warn(
                        "Given {0} interaction filter. Cannot filter interactions in Vina mode. Ignoring filter."
                        .format(opt))
                    continue
                found_res = []
                for res in res_list:
    ...
    def _process_sources(self):
        ...
        if sources['file'] is not None:
            self.lig_files_pool = [
                file for file_list in sources['file'] for file in file_list
        ...
        if len(self.lig_files_pool) > 0 or len(self.rec_files_pool) > 0:
            ...
            if self.save_receptor:
                logging.info("-Found %d receptor files." %
                             len(self.rec_files_pool))
    ...
    def scan_dir(self, path, pattern, recursive=False):
        ...
        """
        logging.info("-Scanning directory [%s] for files (pattern:|%s|)" %
                     (path, pattern))
        files = []
        if recursive:
    ...
    def scan_file_list(self, filename, pattern=".dlg", find_rec=True):
        ...
        if len(lig_accepted) + len(rec_accepted) == 0:
            raise OptionError(
                "*ERROR* No valid files were found when reading from |%s|" %
                filename)
        logging.info("# [ %5.3f%% files in list accepted (%d) ]" %
